...
FilterTwoPole::FilterTwoPole( float frequency0, float qualityFactor, float xInit ) {
	setup(frequency0, qualityFactor, xInit);
}
void FilterTwoPole::setup(float frequency0, float qualityFactor, float xInit ){
...
void FilterTwoPole::print() {
  ...
  Out.print(" Vprev: ");    Out.print( Vprev );
  Out.println("");
}
  ...
  while( true ) {
   ...
   // analogWrite(10,osc.output() ); // hardcoded the dial pin
    ...
    if( now > nextupdateTime ) {
      ...
      Out.print("inputValue: "); Out.print( inputValue );
      Out.print("\t output: "); Out.print( osc.output() );
      Out.println();
    }
  }
}
